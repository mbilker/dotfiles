...
vec4 hook()
{
  ...
  vec2 mypos;
  mypos.y = 2.0*(HOOKED_pos.y - 0.5);
  mypos.x =(HOOKED_pos.x - 0.25); if (HOOKED_pos.x > 0.5) mypos.x = (HOOKED_pos.x - 0.75);
  mypos.x *= 4.0;
  ...
  if (HOOKED_pos.x > 0.5) newx = abs(0.75 + offset_x / 4.0);
  float offset_y = r * f_r * sin(alpha);
  float newy = abs(0.5 + offset_y / 2.0);
  
  float startx = 0.0;
  if (HOOKED_pos.x > 0.5) startx = 0.5;
  ...
  vec4 p = texture(texture0, vec2(newx, newy));
